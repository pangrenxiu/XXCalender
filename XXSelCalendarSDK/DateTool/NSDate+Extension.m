...
{
    return [NSDate date:[NSString stringWithFormat:@"%ld-%02ld-01",self.year,self.month] WithFormat:@"yyyy-MM-dd"];
}

- (NSString *)chineseCalendarDateStr
{
    if ([SKCalendarManage manage].month != self.month) {
        [[SKCalendarManage manage] checkThisMonthRecordFromToday:self];
    }
    return [SKCalendarManage manage].chineseCalendarDate[self.day+self.firstDateOfTheMonth.weekday-2];
}

- (NSString *)chineseCalendarDayStr
{
    if ([SKCalendarManage manage].month != self.month) {
        [[SKCalendarManage manage] checkThisMonthRecordFromToday:self];
    }
    return [SKCalendarManage manage].chineseCalendarDay[self.day+self.firstDateOfTheMonth.weekday-2];
}
